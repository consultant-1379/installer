if (object_id('generate_rbs_combined_view') is not null) then
    drop    procedure generate_rbs_combined_view ;

    if (object_id('dwhrep.generate_rbs_combined_view') is not null) then
		message '<<< FAILED to drop procedure dwhrep.generate_rbs_combined_view >>>' type info to log ;
		message '<<< FAILED to drop procedure dwhrep.generate_rbs_combined_view >>>' type info to client ;
    else
        message '<<< Dropped procedure dwhrep.generate_rbs_combined_view >>>' type info to log ;
		message '<<< Dropped procedure dwhrep.generate_rbs_combined_view >>>' type info to client ;
    end if ;
end if ;

CREATE PROCEDURE dwhrep.generate_rbs_combined_view( IN @table1 VARCHAR(256), IN @table2 VARCHAR(256), IN @viewname VARCHAR(256))
begin
...
set @sql='create view dc.' + @viewname + ' as '||CHAR(10);
